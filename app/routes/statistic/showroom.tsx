...
export let loader: LoaderFunction = async ({ params, request }) => {
	...
	try {
		const statistic = await API.get(session, `${CONFIG.base_url_api.showroom}/statistic`);
		return {
			session: session,
			statistic: statistic,
			isError: false,
...
export default function Index() {
	...
	const navigation = [{ title: "Showroom", href: "", active: true }];
		...
		<div>
			...
			<div className="flex flex-wrap my-5">
				<Card className="bg-blue-500">
					...
					<p className="font-normal text-white">{loader.statistic.totalShowroom || 0} Showroom</p>
				</Card>
				<Card className="bg-teal-500">
					<ShoppingCartIcon className="text-white group-hover:text-white mr-3 flex-shrink-0 h-6 w-6" />
					<p className="font-normal text-white">{loader.statistic.totalOrder || 0} Pesanan</p>
				</Card>
