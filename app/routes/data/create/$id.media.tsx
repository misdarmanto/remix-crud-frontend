...
export let action: ActionFunction = async ({ request }) => {
	...
	try {
		...
		if (request.method == "PATCH") {
			...
			return redirect(`/product/create/${payload.id}/variant`);
		}
	} catch (error: any) {
		console.log(error);
		return { ...error, isError: true };
...
export default function Index(): ReactElement {
		...
		<div className="w-full">
			...
			{thumbnail && (
				<div className="mt-2 relative p-2 bg-white border-dashed border-2 border-gray-400 w-full rounded-lg image-cover flex flex-row justify-center">
					...
					<img
						src={thumbnail}
						className="object-contain h-64 w-96 flex-center rounded-md hover:opacity-80"
					/>
	...
	return (
		<div className="">
			<Breadcrumb title="varian showroom" navigation={navigation} />
			<Form
				method="patch"
				onSubmit={(e): void => {
					submit(e.currentTarget, {
						method: "patch",
						action: `/product/create/${loader.product_id}/media`,
					});
				}}
			>
