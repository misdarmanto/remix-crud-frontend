...
import { IRelawanCreateRequestModel } from "~/models/relawanTimModel";
...
export let loader: LoaderFunction = async ({ params, request }) => {
	...
	const relawanTim = await API.get(
		session,
		CONFIG.base_url_api + `/relawan-tim/detail/${params.id}`
	);
	return { isError: false, session, relawanTim };
};
...
export let action: ActionFunction = async ({ request }) => {
	...
	try {
		if (request.method == "PATCH") {
			const payload: IRelawanCreateRequestModel | any = {
				relawanTimName: formData.get("relawanTimName"),
				relawanTimId: formData.get("relawanTimId"),
			};
			await API.patch(session, CONFIG.base_url_api + "/relawan-tim", payload);
			return redirect("/relawan-tim");
		}
				...
				<div className="w-full md:mr-2">
					<div className="my-6">
						...
						<input
							...
							required
							placeholder="nama..."
							defaultValue={loader.relawanTim.relawanTimName}
						/>
					</div>
				</div>
				<input
					hidden
					name="relawanTimId"
					value={loader.relawanTim.relawanTimId}
				/>
