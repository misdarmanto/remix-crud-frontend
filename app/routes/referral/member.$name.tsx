...
export let loader: LoaderFunction = async ({ params, request }) => {
  ...
  try {
    ...
    return {
      table: {
        link: `relawan-tim/member/${params.name}`,
        data: result,
        page: page,
        size: size,
        filter: {
  ...
  try {
    ...
    if (request.method == 'DELETE') {
      reponse = await API.delete(
        session,
        CONFIG.base_url_api + `/relawan-tim?relawanTimId=${formData.get('relawanTimId')}`
      )
    }
    ...
    <div className=''>
      ...
      >
        <Form method='delete' onSubmit={submitDeleteData}>
          <input className='hidden' name='relawanTimId' value={modalData?.relawanTimId} />
