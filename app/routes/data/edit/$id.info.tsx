	...
	try {
		if (request.method == "PATCH") {
			const pyload = {
				...
				supply_month: formData.get("supply_month"),
			...
			return redirect(`/product/edit/${pyload.id}/media`);
		}
	} catch (error: any) {
		console.log(error);
		return { ...error, isError: true };
...
export default function Index(): ReactElement {
	...
	const [category, setCategory] = useState<CategoryTypes>({
		category_id: loader.product.category_id,
		category_name: loader.product.category_name,
	});
				...
				<div className="my-2 flex justify-between">
					<div className="w-full mr-2">
						...
						<input
							...
							defaultValue={loader.product.name}
							name="product_name"
							required={true}
						/>
					</div>
					<div className="w-full ml-2">
						...
						<input
							...
							type="number"
							min={0}
							defaultValue={loader.product.supply_month}
							placeholder="Masukkan suplay brang"
						/>
					</div>
				</div>
			</div>
			...
			<div className="w-full mb-4">
				...
				<div className="mt-2">
					<textarea
						className={`block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm`}
						name="description"
						defaultValue={loader.product.description}
	...
	const sharingFee = JSON.parse(loader.product.sharing_fee);

	const sharingReferalLevels: ReferralFeeTypes[] =
		sharingFee && Object.values(sharingFee).map((value, i) => ({ level: i + 1, value: value }));
			...
			<Form
				...
				onSubmit={(e) => {
					submit(e.currentTarget, {
						method: "patch",
						action: `/product/edit/${loader.product.id}/info`,
					});
				}}
			>
				{loader.address === null && (
					...
					<div className="w-full md:flex mt-4">
						...
						<div className="w-full md:w-1/2">
							...
							<div className="flex items-center mt-5 ">
								...
								<input
									name="platform_fee"
									defaultValue={loader.product.platform_fee}
									type="number"
